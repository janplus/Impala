...
BooleanVal AndPredicate::StatisticsJudge(ExprContext* context, const TupleRow* row) {
  DCHECK_EQ(children_.size(), 2);
  BooleanVal val1 = children_[0]->StatisticsJudge(context, row);
  if (!val1.is_null && !val1.val) return BooleanVal(false); // short-circuit

  BooleanVal val2 = children_[1]->StatisticsJudge(context, row);
...
BooleanVal OrPredicate::StatisticsJudge(ExprContext* context, const TupleRow* row) {
  DCHECK_EQ(children_.size(), 2);
  BooleanVal val1 = children_[0]->StatisticsJudge(context, row);
